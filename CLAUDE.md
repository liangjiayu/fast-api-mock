...
Mock API 服务，使用 Hono + Zod OpenAPI 构建，部署在 Netlify Functions 上。返回硬编码的模拟数据（用户信息、商品列表），用于前端开发联调。
...
- `src/app.ts` — 应用入口，注册所有路由，挂载 OpenAPI 文档 (`/doc`) 和 Swagger UI (`/ui`)。本地开发通过 `@hono/node-server` 启动，生产环境通过 Netlify Functions。
- `src/routes/` — 每个文件导出一个 `createRoute()` 定义和对应的 handler 函数。路由定义与 handler 在同一文件中。
- `src/schemas/` — Zod schemas，使用 `.openapi()` 扩展。`common.ts` 提供分页等通用 schema 工厂函数（如 `createPaginatedResponse`）。
